...
def choose_expert(mvn_fit, consensus_model, dataset):
    ...
    for c in candidates:
        ...
        for j in range(1, K+1):
            ...
            # ind = (1-1)*K + j -1
            # print("confidence of candidate 1: " + str(hyp_fit.stan_variable("L_H")[:,ind].mean()))
            # ind = (3-1)*K + j -1
            # print("confidence of candidate 3: " + str(hyp_fit.stan_variable("L_H")[:,ind].mean()))
...
def main():
    ...
    dataset = Dataset(
        ...
        model_predictions_train = data_dict['Y_M'],
        human_predictions_train = data_dict['Y_H'],
        model_predictions_test = data_dict['Y_M_new_list'],
        human_predictions_test = data_dict['Y_H_new_list'],
        apply_temperature_scaling = apply_TS
    )

    data_dict = dataset.get_init_data_dict(subset_size=n_items)
